/*
 * A fblock that generates random_ubx numbers.
 *
 ...
 */
#include "types/random_ubx_config.h"
#include "types/random_ubx_config.h.hexarr"

/* declare the type and give the char array type representation as the type private_data */
ubx_type_t random_ubx_config_type = def_struct_type(struct random_ubx_config, &random_ubx_config_h);

/* function block meta-data
 * used by higher level functions.
 */
char rnd_meta[] =
	"{ doc='A random_ubx number generator function block',"
	"  license='LGPL',"
	"  real-time=true,"
	"}";
...
ubx_config_t rnd_config[] = {
	{ .name="min_max_config", .type_name = "struct random_ubx_config" },
	{ NULL },
};
...
static int rnd_init(ubx_block_t *b)
{
	...
	DBG(" ");
	if ((b->private_data = calloc(1, sizeof(struct random_ubx_info)))==NULL) {
...
/**
 * rnd_start - start the random_ubx block.
 *
 * @param b
 *
...
static int rnd_start(ubx_block_t *b)
{
	...
	float ret;
	unsigned int clen;
	struct random_ubx_config* rndconf;
	struct random_ubx_info* inf;

	inf=(struct random_ubx_info*) b->private_data;

	/* get and store min_max_config */
	rndconf = (struct random_ubx_config*) ubx_config_get_data_ptr(b, "min_max_config", &clen);
	inf->min = rndconf->min;
...
static void rnd_step(ubx_block_t *b) {
	...
	rand_val_float=(float)rand_val*0.15;
	printf("%f", rand_val_float);
	write_float(rand_port, &rand_val_float);
}
 ...
 *
 */
ubx_block_t random_ubx_comp = {
	.name = "random_ubx/random_ubx",
...
{
	ubx_type_register(ni, &random_ubx_config_type);
	return ubx_block_register(ni, &random_ubx_comp);
}
...
{
	ubx_type_unregister(ni, "struct random_ubx_config");
	ubx_block_unregister(ni, "random_ubx/random_ubx");
}
